...
/**
 * Se utiliza para validar el registro de usuario por credenciales
 */
export const AuthBaseSchema = PersonSchema.pick({ email: true }).extend({
  username: z
    .string()
 ...
 */
export const SignInSchema = PersonSchema.pick({ email: true }).extend({
  password: z
    .string()
    .min(8, 'La contraseña debe tener al menos 8 caracteres')
    .max(50, 'La contraseña no puede tener más de 50 caracteres'),
